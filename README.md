...
- **AI-capable surface.** Providers, observations, assistant sessions, web fetch, eval catalog/router, command DSL, tool registry/executor, autopilot runner, and automation plans are first-class features (`src/services/aiService.js`, `src/services/aiAssistantService.js`, `src/services/evalService.js`, `src/services/evalRouterService.js`, `src/services/commandExecutorService.js`, `src/services/toolExecutionService.js`, `src/services/autopilotService.js`). AI events are audit-logged with PII masking and rich traces. See `docs/ai-brain.md` plus `docs/ai/control-center.md` for the agent/provider/tool/eval registry and control APIs. Inventory display config APIs (`/v1/inventory-display-config`), control-center routes (`/v1/ai-control/*`), system evals (market update, self-test, daily briefing, whats-next), command + tool execution via `/v1/ai/assistant/model-output`, and expanded tool coverage (CRM, content/SEO, campaigns/analytics, integrations/ops) are included by default.
